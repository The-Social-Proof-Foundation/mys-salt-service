...
/// Resolve callback URL from `links` JSON using the first matching key in order.
...
mod tests {
    ...
    #[test]
    fn redirect_from_links_first_key_wins() {
        let links = serde_json::json!({ "website": "https://w.test/", "oauthRedirect": "https://o.test/" });
        let keys = vec!["website".into(), "oauthRedirect".into()];
        assert_eq!(
            redirect_uri_from_links(Some(&links), &keys).as_deref(),
            Some("https://w.test/")
        );
